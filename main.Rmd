...
```{r}
aggregate(math.score + reading.score + writing.score ~ gender, data, mean)
```
...
```{r}
aggregate(math.score + reading.score + writing.score ~ race.ethnicity, data, mean)
```
...
```{r}
aggregate(math.score + reading.score + writing.score ~ parental.level.of.education, data, mean)
```

```{r}
heatmap(aggregate(math.score + reading.score + writing.score ~ parental.level.of.education, data, mean))
```
...
```{r}
aggregate(math.score + reading.score + writing.score ~ lunch, data, mean)
```
#### Test Preparation Course

```{r}
aggregate(math.score + reading.score + writing.score ~ test.preparation.course, data, mean)
```
...
If we see on the data above, like it or not, priviledge does exist, parent education, lunch package, and test preparation course have an impact to what makes student more successful than the others. Genetic factors like gender female and etnicity group E have more successful in exams performance.